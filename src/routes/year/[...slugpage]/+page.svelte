...
<Seo title="Daftar Anime {currentStatus[0]} - {config.title}: {config.slogan}" />

<section class="py-5">
  <div class="container mx-auto">
    <div class="flex flex-col space-y-6">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold dark:text-white">Anime {currentStatus[0]}</h1>
      </div>
      ...
      <Pagination {currentPage} {totalPages} baseRoute="/year/{currentStatus[0]}" />
    </div>
  </div>
</section>